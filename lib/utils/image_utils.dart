...
Uint8List applyExifRotation(Uint8List bytes, int exifRotation) {
  final image = image_lib.decodeImage(bytes);
  if (image == null) {
    return bytes;
  }
  if (exifRotation == 1) {
    return image_lib.copyRotate(image, angle: 0).toUint8List();
  } else if (exifRotation == 3) {
    return image_lib.copyRotate(image, angle: 180).toUint8List();
  } else if (exifRotation == 6) {
    return image_lib.copyRotate(image, angle: 90).toUint8List();
  } else if (exifRotation == 8) {
    return image_lib.copyRotate(image, angle: 270).toUint8List();
  }
