...
class LivenessDetectionView extends StatefulWidget {
  ...
  final Future<bool> Function(String?)? onLivenessSuccessStep;
  final Future<void> Function()? onLivenessResetStep;
...
class _LivenessDetectionScreenState extends State<LivenessDetectionView> {
  ...
  static void shuffleListLivenessChallenge({
    ...
    bool isStraightFaceLast = true,
  }) {
    if (isStraightFaceLast) {
      int? straightFaceIndex = list.indexWhere(
          (item) => item.step == LivenessDetectionStep.straightFace);
      if (straightFaceIndex != -1) {
        LivenessDetectionStepItem straightFaceItem =
            list.removeAt(straightFaceIndex);
        list.shuffle(Random());
        list.add(straightFaceItem);
      }
    } else if (isSmileLast) {
      int? blinkIndex =
  ...
  List<LivenessDetectionStepItem> customizedLivenessLabel(
    ...
    if (!_isShuffled) {
      ...
      if (label.straightFace != "" && widget.config.useCustomizedLabel) {
        customizedSteps.add(LivenessDetectionStepItem(
          step: LivenessDetectionStep.straightFace,
          title: label.straightFace ?? "Straight Face",
        ));
      }
  ...
  void _detectFace({
    ...
    switch (step) {
      ...
      case LivenessDetectionStep.smile:
        ...
        break;
      case LivenessDetectionStep.straightFace:
        await _handlingStraightFace(face: face, step: step);
        break;
    }
  }
  ...
  Future<void> _completeStep({required LivenessDetectionStep step}) async {
    ...
    final result = await widget.onLivenessSuccessStep?.call(image?.path);
    if (result == false) {
      return;
    }
    if (mounted) setState(() {});
  ...
  void _resetSteps() {
    if (widget.config.useCustomizedLabel) {
      ...
      if (_stepsKey.currentState?.currentIndex != 0) {
        ...
        widget.onLivenessResetStep?.call();
      }
      if (mounted) setState(() {});
    } else {
  ...
  Future<void> _handlingLookDown({
    ...
    final headTurnThreshold = FlutterLivenessDetectionRandomizedPlugin
        ...
        as LivenessThresholdHead?;
    if ((face.headEulerAngleX ?? 0) <
        (headTurnThreshold?.rotationAngle ?? -12)) {
      _startProcessing();
      await _completeStep(step: step);
    }
  }
  ...
  Future<void> _handlingStraightFace({
    required Face face,
    required LivenessDetectionStep step,
  }) async {
    final straightThreshold = FlutterLivenessDetectionRandomizedPlugin
            .instance.thresholdConfig
            .firstWhereOrNull((p0) => p0 is LivenessThresholdStraightFace)
        as LivenessThresholdStraightFace?;

    final double maxDeviation = straightThreshold?.maxDeviationAngle ?? 10.0;
    ...
    final bool isStraight =
        yaw <= maxDeviation && pitch <= maxDeviation && roll <= maxDeviation;
