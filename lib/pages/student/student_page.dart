...
import 'blocs/student_state.dart';
...
class _StudentPageState extends State<StudentPage> {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider<StudentBloc>(create: (_) => _bloc),
  ...
  Future<void> _confirmAndSyncStudents() async {
    ...
    if (confirmed == true) {
      await _syncLocalStudentsToServer();
    }
  }
  ...
  Future<void> _showItemOptions(Student? student) async {
    ...
    switch (action) {
      ...
      case 'edit_name':
        ...
        if (newName != null && newName.trim().isNotEmpty) {
          _bloc.onUpdateStudentInList(student.copyWith(name: newName.trim()));
        }
        break;
      case 'edit_position':
        final newPos = await _showEditInputDialog(
          title: 'Cập nhật lớp',
          label: 'Lớp mới',
          initialValue: student.jobTitle ?? '',
        );
        if (newPos != null && newPos.trim().isNotEmpty) {
          _bloc
              .onUpdateStudentInList(student.copyWith(jobTitle: newPos.trim()));
        }
        break;
      case 'edit_pin':
        final newPin = await _showEditInputDialog(
          ...
          initialValue: student.pin ?? '',
          keyboardType: TextInputType.number,
        );
        if (newPin != null && newPin.trim().isNotEmpty) {
          await _bloc.onUpdateStudentInList(
              student.copyWith(pin: newPin.trim()),
              isUpdatePin: true);
  ...
  // Sync students local data to server
  Future<void> _syncLocalStudentsToServer() async {
    _bloc.syncData();
  }
}
