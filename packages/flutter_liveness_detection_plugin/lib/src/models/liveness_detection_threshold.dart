...
class LivenessThresholdStraightFace extends LivenessDetectionThreshold {
  final double maxDeviationAngle;

  LivenessThresholdStraightFace({
    this.maxDeviationAngle = 10.0,
  }) : super() {
    assert(
      maxDeviationAngle > 0.0 && maxDeviationAngle <= 180.0,
      'Max deviation angle must be > 0 and ≤ 180 degrees.',
    );
  }

  LivenessThresholdStraightFace copyWith({
    double? maxDeviationAngle,
  }) {
    return LivenessThresholdStraightFace(
      maxDeviationAngle: maxDeviationAngle ?? this.maxDeviationAngle,
    );
  }
  ...
  Map<String, dynamic> toMap() {
    ...
    result.addAll({'maxDeviationAngle': maxDeviationAngle});

    return result;
  }

  factory LivenessThresholdStraightFace.fromMap(Map<String, dynamic> map) {
    return LivenessThresholdStraightFace(
      maxDeviationAngle: map['maxDeviationAngle']?.toDouble() ?? 0.0,
    );
  }

  String toJson() => json.encode(toMap());

  factory LivenessThresholdStraightFace.fromJson(String source) =>
      LivenessThresholdStraightFace.fromMap(json.decode(source));

  @override
  String toString() =>
      'LivenessThresholdStraightFace(maxDeviationAngle: $maxDeviationAngle)';

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;

    return other is LivenessThresholdStraightFace &&
        other.maxDeviationAngle == maxDeviationAngle;
  }

  @override
  int get hashCode => maxDeviationAngle.hashCode;
